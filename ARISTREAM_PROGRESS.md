# AriStream Streaming Platform — Build Progress

> Last updated: 2026-04-19

## What We're Building

A **Restream.io-like** multi-platform streaming platform at **aristream.com**:
...
- Viewers watch at `{slug}.aristream.com/live`
...
| Component | Details |
|-----------|---------|
| VM Host | 157.173.124.222 |
| Docker image | `aristream-backend:latest` (built locally / on VM) |
| Source on VM | `/home/stream/tribenest` |
| Docker Compose | `/home/stream/tribenest/docker-compose.contabo.yml` |
| Cloudflare Tunnel | erp-cloudflared tunnel → aristream-backend:5000 |
| Redis | standalone container, port 6379 (on host) |
...
ROOT_DOMAIN=aristream.com NEXT_PUBLIC_API_URL=https://api.aristream.com npx next build
...
cd /opt/tribenest
docker-compose -f docker-compose.aristream.yml up -d --force-recreate
```
...
- [ ] Add OAuth credentials to `docker-compose.aristream.yml`:
  ```yaml
  GOOGLE_CLIENT_ID: "..."
...
## Environment Variables (docker-compose.aristream.yml)

### Currently set
```yaml
MULTI_TENANT: "true"
JWT_SECRET: "aristream-secret-jwt-2026"
CREATOR_PORTAL_URL: "https://aristream.com"
GOOGLE_REDIRECT_URI: "https://aristream.com/oauth/youtube"
TWITCH_REDIRECT_URI: "https://aristream.com/oauth/twitch"
...
- Google: `https://aristream.com/oauth/youtube`
- Twitch: `https://aristream.com/oauth/twitch`
...
6. Viewers on `{slug}.aristream.com/live` see the `<video>` player with the stream

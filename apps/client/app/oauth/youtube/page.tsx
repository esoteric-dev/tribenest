...
function YouTubeOAuthContent() {
  ...
  useEffect(() => {
    ...
    const profileId = params.get("state"); // we pass profileId as state
    ...
    fetch(`${API_URL}/streams/oauth/youtube/token?code=${encodeURIComponent(code)}&profileId=${profileId}`, {
      ...
      .then((data) => {
        ...
        setTimeout(() => {
          // Find slug from authorizations and redirect
      ...
      .catch((err) => {
        setStatus("error");
        setMessage(err.message || "Failed to connect YouTube. Please try again.");
      });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
      <div className="text-center">
        <div className="w-16 h-16 rounded-2xl bg-red-600 flex items-center justify-center mx-auto mb-6 text-3xl">▶</div>
        {status === "loading" && (
...
export default function YouTubeOAuthPage() {
  return <Suspense><YouTubeOAuthContent /></Suspense>;
}
